...
标准工具栏从依赖具体 `KLineSceneRuntime` 改为依赖 `DrawingRuntimeCapability` 和 Runtime 提供的显式能力描述。两种场景都完整提供 22 种工具、线型、线宽、颜色、文本、选择和删除；这些核心 Drawing 控件不存在场景子集。

数值轴设置和导出属于可选的非 Drawing 控件。K 线 Runtime 声明线性与对数轴能力，TimeSeriesScene v1 只声明线性轴；工具栏根据能力描述决定是否展示可操作的轴切换，不通过 Scene 类型分支判断。旧模式的导出能力明确产生原有 `ChartScene` 制品，工作区模式的导出能力明确产生 `DrawableWorkspaceDocument` 制品；工具栏只执行 Runtime 声明的制品导出，不做格式迁移或自动包装。
...
现有 K 线工具栏调用方式保持成立，并继续获得原有控件和 `ChartScene` 导出语义。时间序列画图与新 K 线工作区使用同一个标准工具栏组件连接 `DrawableWorkspaceRuntime`。两类入口在类型和制品种类上保持显式，不能根据运行时数据自动切换 legacy Scene 模式和 Drawing Workspace 模式。
