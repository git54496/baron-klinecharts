...
const publicPackageVersions = new Map([
	...
	['@baron1996/klinecharts-runtime', '0.9.3'],
...
test('four public tarballs support the M1 and M2 Runtime journeys through package exports', async () => {
	...
	await writeFile(
		...
		[
			...
			"if (adapter.ADAPTER_PACKAGE_VERSION !== '0.9.3') throw new Error('Adapter root export failed.');",
			"if (runtime.WEB_RUNTIME_PACKAGE_VERSION !== '0.9.3') throw new Error('Runtime root export failed.');",
	...
	try {
		...
		assert.equal(readiness.adapterVersion, '0.9.3');
