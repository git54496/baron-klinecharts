...
const publicPackageVersions = new Map([
	...
	['@baron1996/klinecharts-adapter', '0.9.20'],
	['@baron1996/klinecharts-runtime', '0.9.20'],
	['@baron1996/klinecharts-cli', '0.9.20'],
]);
...
test('four public tarballs support the M1 and M2 Runtime journeys through package exports', async () => {
	...
	await writeFile(
		...
		[
			...
			"if (schema.SCENE_PACKAGE_VERSION !== '0.9.20') throw new Error('Schema root export failed.');",
			"if (adapter.ADAPTER_PACKAGE_VERSION !== '0.9.20') throw new Error('Adapter root export failed.');",
			"if (runtime.WEB_RUNTIME_PACKAGE_VERSION !== '0.9.20') throw new Error('Runtime root export failed.');",
	...
	try {
		...
		assert.equal(readiness.adapterVersion, '0.9.20');
