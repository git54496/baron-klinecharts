...
test('packed npm artifacts install and run without workspace links', async () => {
	...
	await writeFile(
		...
		[
			...
			"if (WEB_RUNTIME_PACKAGE_VERSION !== '0.9.9') throw new Error('Web Runtime import failed.');",
			"if (ADAPTER_PACKAGE_VERSION !== '0.9.9') throw new Error('Adapter import failed.');",
	...
	await writeFile(
		...
		[
			...
			"if (WEB_RUNTIME_PACKAGE_VERSION !== '0.9.9') throw new Error('Time Series Runtime import failed.');",
