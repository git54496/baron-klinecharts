...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.8.0
```
...
`@baron1996/klinecharts-runtime@0.8.0` 和 `baron-klinecharts==0.6.0`。升级只能
...
当前发布版本为 `0.8.0`。本次发布 Adapter 与 Web Runtime `0.8.0`；Scene Schema、
CLI、Python 和私有 Render Runtime 继续保持 `0.6.0`，所有内部依赖使用精确版本。
该版本新增由宿主接管网络请求的更早行情端口：K 线到达左边界时发出纯数据请求，
宿主提交更早数据后原位前插并保持可见位置；失败可显式释放请求并重试。该能力不
提升 Drawing/Workspace Schema 版本或 Runtime 事件协议版本。
