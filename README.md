...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.7.2
```
...
`@baron1996/klinecharts-runtime@0.7.2` 和 `baron-klinecharts==0.6.0`。升级只能
...
当前发布版本为 `0.7.2`。本次只发布 Web Runtime `0.7.2`；Scene Schema、Adapter、
CLI、Python 和私有 Render Runtime 继续保持 `0.6.0`，所有内部依赖使用精确版本。
该版本修复 Drawing 候选重建时丢失文档级 metadata、Drawing metadata 与 groupId 的
问题，不提升 Drawing/Workspace Schema 版本或 Runtime 事件协议版本。
