...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.9.14
```

安装 CLI：

```bash
npm install --global @baron1996/klinecharts-cli@0.9.14
...
`@baron1996/klinecharts-runtime@0.9.14` 和 `baron-klinecharts==0.9.14`。升级只能
...
```bash
pip install baron-klinecharts==0.9.14
...
当前发布版本为 `0.9.14`。本次同步发布 Web Runtime、Adapter、Scene Schema、CLI 和
Python，私有 Render Runtime 也同步到 `0.9.14`，所有内部依赖使用精确版本。该版本新增
专业图表工作区复合工具栏，将周期与展示控制放在顶部、Drawing 工具放在左侧，并提供
主图 MA、EMA、SMA、BOLL、SAR、BBI 指标、动态显示时区、设置归类和全屏入口。主图指标
由浏览器基于当前 OHLCV 计算，跨周期替换 Scene 时默认保留用户选择；时区切换只改变
显示，不触发行情重取或改变 Drawing 坐标。该能力不提升 Drawing/Workspace Schema
版本或 Runtime 事件协议版本。
