...
线性与对数价格轴不共享 `DrawingDocument`。v2 文档把 `scale` 纳入数值轴坐标身份，
即使文档为空也必须与当前 Scene 匹配；v1 文档仍可作为未声明坐标轴类型的历史数据读取。
宿主负责按自己的业务维度（例如标的、复权类型）再加 `scale` 保存和加载独立文档。
复合工具栏允许宿主提供两种坐标轴各自的标注数量，并在对应入口显示画笔图标；价格轴
点击也可交给宿主完成保存当前文档、加载目标文档和整体替换 Workspace 的原子切换。
