...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.9.18
```

安装 CLI：

```bash
npm install --global @baron1996/klinecharts-cli@0.9.18
...
`@baron1996/klinecharts-runtime@0.9.18` 和 `baron-klinecharts==0.9.18`。升级只能
...
```bash
pip install baron-klinecharts==0.9.18
...
当前发布版本为 `0.9.18`。本次同步发布 Web Runtime、Adapter、Scene Schema、CLI 和
Python，私有 Render Runtime 也同步到 `0.9.18`，所有内部依赖使用精确版本。该版本将
专业图表工作区的“清空全部标注”收敛为一次原子批量变更：所有未锁定 Drawing 共同形成
一份候选文档，只触发一次宿主持久化和一次 Runtime commit；宿主拒绝候选时则恢复完整
前态，锁定 Drawing 始终保留。该修复不改变现有宿主持久化接口，也不提升
