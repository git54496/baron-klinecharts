...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.9.12
```

安装 CLI：

```bash
npm install --global @baron1996/klinecharts-cli@0.9.12
...
`@baron1996/klinecharts-runtime@0.9.12` 和 `baron-klinecharts==0.9.12`。升级只能
...
```bash
pip install baron-klinecharts==0.9.12
...
当前发布版本为 `0.9.12`。本次同步发布 Web Runtime、Adapter、Scene Schema、CLI 和
Python，私有 Render Runtime 也同步到 `0.9.12`，所有内部依赖使用精确版本。该版本在
