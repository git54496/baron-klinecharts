...
```bash
npm install --save-exact @baron1996/klinecharts-runtime@0.2.3
```

安装 CLI：

```bash
npm install --global @baron1996/klinecharts-cli@0.2.3
...
`@baron1996/klinecharts-runtime@0.2.3` 和 `baron-klinecharts==0.2.3`。升级只能
...
```bash
pip install baron-klinecharts==0.2.3
...
当前发布候选协调版本为 `0.2.3`。Scene Schema、Adapter、Web Runtime、CLI、
Python 和私有 Render Runtime 统一为 `0.2.3`，所有内部依赖使用精确版本。
